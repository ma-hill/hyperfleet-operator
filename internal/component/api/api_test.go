...
// spec.replicas and rollout status fields set, for feeding directly into
// Conditions without any cluster/client dependency. ObservedGeneration matches
// generation (the Deployment controller has caught up); targeted tests override
// individual status fields after construction to simulate incomplete rollouts.
func deploymentWithStatus(specReplicas, statusReplicas, availableReplicas, updatedReplicas int32, generation int64) *appsv1.Deployment {
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{Name: ResourceName, Generation: generation},
		Spec:       appsv1.DeploymentSpec{Replicas: ptr.To(specReplicas)},
...
func TestConditionsZeroAvailableReplicas(t *testing.T) {
	...
	dep := deploymentWithStatus(1, 1, 0, 0, 1)
...
func TestConditionsPartiallyReady(t *testing.T) {
	...
	dep := deploymentWithStatus(2, 2, 1, 1, 1)
	conds, err := New("img", testNamespace, Options{}).Conditions(context.Background(), testCR(), []client.Object{dep})
	g.Expect(err).NotTo(HaveOccurred())
...
func TestConditionsFullyReadyAndStable(t *testing.T) {
	...
	dep := deploymentWithStatus(1, 1, 1, 1, 1)
	conds, err := New("img", testNamespace, Options{}).Conditions(context.Background(), testCR(), []client.Object{dep})
	g.Expect(err).NotTo(HaveOccurred())
...
func TestConditionsMidRollout(t *testing.T) {
	...
	dep := deploymentWithStatus(1, 1, 1, 1, 2)
...
func TestConditionsRolloutStillProgressingWhileOldReplicasRemain(t *testing.T) {
	g := NewWithT(t)

	// All desired replicas are updated, but an old surge replica still exists.
	dep := deploymentWithStatus(1, 2, 1, 1, 1)
	conds, err := New("img", testNamespace, Options{}).Conditions(context.Background(), testCR(), []client.Object{dep})
	g.Expect(err).NotTo(HaveOccurred())
...
func TestConditionsRolloutStillProgressingWhileUpdatedReplicasAreUnavailable(t *testing.T) {
	g := NewWithT(t)

	// The new ReplicaSet has been fully updated, but the updated pod is not yet
	// available, so rollout is not complete.
	dep := deploymentWithStatus(1, 1, 0, 1, 1)
	conds, err := New("img", testNamespace, Options{}).Conditions(context.Background(), testCR(), []client.Object{dep})
	g.Expect(err).NotTo(HaveOccurred())
