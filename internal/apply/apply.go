...
// After apply, the object is re-read in place through refreshReader so callers
// can inspect the live state the API server now holds (including status
// written by other controllers) without issuing their own Get or depending on
// cache read-after-write coherence. Errors are wrapped with the object's kind
// and name so the caller can see which operand failed.
func Objects(
	ctx context.Context,
	c client.Client,
	refreshReader client.Reader,
	...
	for _, obj := range objs {
		...
		if err := refreshReader.Get(ctx, client.ObjectKeyFromObject(obj), obj); err != nil {
