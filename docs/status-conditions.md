...
Each condition's `observedGeneration` reflects the `.metadata.generation` that
specific condition was last evaluated against. The top-level
`status.observedGeneration` reflects the newest generation for which the
operator fully collected component health and updated the rollup status.
`lastTransitionTime` changes only when a condition's `status` actually flips —
reconciles that leave health unchanged do not touch it.
...
`Available`, `Progressing`, and the top-level `status.observedGeneration` are
left exactly as they were last recorded rather than being guessed. Only
`Degraded` is re-evaluated for the new generation. This avoids publishing a
fabricated "healthy" value alongside `Degraded=True` when the real state was
